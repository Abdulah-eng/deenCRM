...
export default function SalesCustomersPage() {
  ...
  const openEdit = (c) => { setForm({ name: c.name || '', email: c.email || '', contact: c.contact_person || c.contact || '', phone: c.phone || '', city: c.city || '', vat13b: c.vat13b || false, assigned_company: c.assigned_company || 'All', status: (c.status || 'ACTIVE').toUpperCase() }); setEditTarget(c.id); setShowModal(true); };
  ...
  const handleSave = async () => {
    ...
    const dataToSave = {
      ...
      city: form.city,
      vat13b: form.vat13b,
      assigned_company: form.assigned_company,
      status: form.status
    };

    let error;
    if (editTarget) {
      const { error: err } = await supabase.from('customers').update(dataToSave).eq('id', editTarget);
      error = err;
    } else {
      const { error: err } = await supabase.from('customers').insert([dataToSave]);
      error = err;
    }
    ...
    if (error) {
      ...
      if (editTarget) {
        await supabase.from('customers').update(coreData).eq('id', editTarget);
      } else {
        await supabase.from('customers').insert([coreData]);
      }
    }
  ...
  const modalHeader = { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px 24px', borderBottom: '1px solid var(--card-border)', background: 'linear-gradient(135deg, rgba(16,185,129,0.06), rgba(16,185,129,0.02))' };
  ...
  const closeBtn = { width: 32, height: 32, borderRadius: 8, border: '1px solid var(--card-border)', background: 'var(--body-bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: 'var(--body-text)' };
  ...
  const saveBtnStyle = { padding: '9px 18px', borderRadius: 8, border: 'none', background: '#10b981', color: 'white', fontSize: 13, fontWeight: 600, cursor: 'pointer' };
  ...
  return (
    <>
      <Header title="Kunden" subtitle="Vertrieb / Kunden" />
      <div className={styles.container}>
        ...
        <div className={styles.kpiGrid}>
          <div className={styles.kpiCard}><div className={styles.kpiValue} style={{ color: '#10b981' }}>{customers.length}</div><div className={styles.kpiLabel}>Gesamte Kunden</div></div>
          ...
          <div className={styles.kpiCard}><div className={styles.kpiValue} style={{ color: '#10b981' }}>{customers.length}</div><div className={styles.kpiLabel}>Insgesamt gelistet</div></div>
        </div>

        <div className={styles.tableCard}>
          <div className={styles.tableHeader}>
            <div className={styles.tableTitle}><Users size={16} color="#10b981" /> Alle Kunden ({filtered.length})</div>
                  ...
                  {filtered.map((c, index) => {
                    ...
                    return (
                      <tr key={c.id}>
                        ...
                        <td><span className={styles.cellPrimary}>{c.contact_person || c.contact || '—'}</span><span className={styles.cellSecondary}>{c.phone || '—'}</span></td>
                        ...
                        <td>
                          <span className={cStatus === 'ACTIVE' ? styles.badgeGreen : styles.badgeRed}>
                            {cStatus === 'ACTIVE' ? 'AKTIV' : 'INAKTIV'}
                          </span>
                        </td>
                        <td>
                          <div className={styles.actionWrapper}>
                            <button className={styles.actionBtn} onClick={() => openEdit(c)}><Edit3 size={13} color="#10b981" /></button>
        ...
        <div style={overlay}>
          <div style={modalBox}>
            <div style={modalHeader}>
              <h3 style={{ margin: 0, fontSize: 16, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 8, color: 'var(--header-text)' }}>
                <Plus size={16} color="#10b981" /> {editTarget ? 'Kunde bearbeiten' : 'Neuen Kunden hinzufügen'}
              </h3>
      ...
      {deleteTarget && (
        <div style={overlay}>
          <div style={{ ...modalBox, maxWidth: 380, padding: 36, textAlign: 'center' }}>
            ...
            <h3 style={{ margin: '0 0 8px', fontSize: 18, color: 'var(--header-text)' }}>Kunden löschen?</h3>
