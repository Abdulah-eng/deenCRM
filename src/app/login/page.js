...
export default function LoginPage() {
  ...
  const [company, setCompany] = useState('Elektro Werke Süddeutschland');
    ...
    <div className={styles.loginContainer}>
      <div className={styles.loginCard}>
        <div className={styles.logoSection}>
          <div className={styles.logoIcon} style={{ background: 'transparent', width: 'auto', height: 'auto', display: 'flex', justifyContent: 'center', marginBottom: 12 }}>
            <img src="/logo.png" alt="ProCRM Logo" style={{ maxHeight: '48px', objectFit: 'contain' }} />
          </div>
          <h1>ProCRM</h1>
          <p>Süddeutsche Werke — Einheitliches Managementsystem</p>
        </div>

        <div className={styles.quickAccess}>
          <p className={styles.dividerText}>TEST-KONTEN (ZUM AUTOMATISCHEN AUSFÜLLEN ANKLICKEN)</p>
        ...
        <form className={styles.loginForm} onSubmit={handleLogin}>
          <p className={styles.dividerText}>ODER MIT BENUTZERDATEN ANMELDEN</p>
          ...
          <div className={styles.inputGroup}>
            ...
            <div className={styles.inputWrapper}>
              ...
              <select className={styles.selectInput} value={role} onChange={(e) => setRole(e.target.value)}>
                ...
                <option value="Sales Staff">Vertriebsmitarbeiter</option>
                <option value="Crew / Worker">Montageteam / Mitarbeiter</option>
                <option value="Accountant / Finance">Buchhaltung / Finanzen</option>
              </select>
              <ChevronDown size={16} className={styles.dropdownIcon} />
            </div>
          </div>
          ...
          <button type="submit" className={styles.submitBtn} disabled={loading}>
            {loading ? 'Authentifizierung...' : <><LogIn size={18} /> Im Dashboard anmelden</>}
          </button>
        </form>
      </div>
    </div>
